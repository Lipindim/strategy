...
public class AutoAttackEvaluator : MonoBehaviour
{
    ...
    private void evaluate(GameObject go, AttackerParallelnfo info)
    {
        ...
        foreach (var factionMemberInfo in FactionMembersInfo)
        {
            if (factionInfo.Faction == factionMemberInfo.Value.Faction)
            {
                continue;
            }

            var distance = Vector3.Distance(factionInfo.Position, factionMemberInfo.Value.Position);
            if (distance > info.VisionRadius)
            {
                continue;
            }

            AutoAttackCommands.OnNext(new Command(go, factionMemberInfo.Key));
            break;
        }
    }
}